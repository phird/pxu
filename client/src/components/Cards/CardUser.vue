...
export default {
  ...
  methods: {
    getem() {
      axios.get("http://128.199.187.173:3000/carddash", {}).then((res) => {
        this.em = res.data[0];
      });
    },
  },
...
.card-profile-section {
  /* div that contain banner and text-section */
  display: flex;
  position: relative;
...
.card-profile-section .profile-banner {
  /*  whole banner div*/
  display: flex;
  width: 100%;
  height: 50%;
...
.card-profile-section .card-body {
  /*  whole text-section   */
  display: flex;
  flex-shrink: 1;
  flex-direction: column;
